...
if (obj_mario.isduck == 1)
{
    ...
    else if (mouthholder == obj_yoshifire_purple)
    {
        audio_play_sound(snd_flames, 0, false)
    ...
    else if (mouthholder == obj_spike_ball)
    {
...
else
{
    ...
    else
    {
        ...
        with (instance_create(vall, (y - 16), mouthholder))
        {
            ...
            else if ((sprite_index == spr_shell) || ((sprite_index == spr_shell_down) || ((sprite_index == spr_shell_red) || ((sprite_index == spr_shell_red_down) || ((sprite_index == spr_shell_buzzy_empty) || (sprite_index == spr_shell_spiny_empty))))))
            {
            ...
            else if (!collision_rectangle(bbox_left, bbox_top, bbox_right, bbox_bottom, obj_solid, 0, 0))
            {
                if (collision_rectangle(bbox_left, bbox_top, bbox_right, bbox_bottom, obj_water_draw, 0, 0) || (global.bg_level == "underwater"))
                {
                    if ((object_index == obj_mushroom) || ((object_index == obj_1up) || (object_index == obj_star)))
                        hspeed = ((0.7 + abs(obj_mario.hspeed)) * sign(obj_mario.direct))
                    else
                        hspeed = ((0.5 + abs(obj_mario.hspeed)) * sign(obj_mario.direct))
                }
                else if ((object_index == obj_mushroom) || ((object_index == obj_1up) || (object_index == obj_star)))
                    hspeed = (1.1 * obj_mario.direct)
                else
                    hspeed = (obj_mario.direct + (2.5 * obj_mario.direct))
            }
            else
                inwall = 1
        }
