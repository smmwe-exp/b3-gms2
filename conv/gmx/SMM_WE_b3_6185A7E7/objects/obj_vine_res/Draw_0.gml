...
for (v = 0; v < size_y; v++)
{
    if (v == 0)
    {
        if collision_rectangle(bbox_left, (bbox_top - 2), bbox_right, (bbox_top - 2), obj_ground, 0, 0)
            index = 1
        else
            index = 0
    }
    ...
    {
        if collision_rectangle(bbox_left, (bbox_bottom + 2), bbox_right, (bbox_bottom + 2), obj_ground, 0, 0)
            index = 1
        else
            index = 2
    }
